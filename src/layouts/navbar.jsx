...
function Navbar() {
  ...
  return (
   ...
   <div className="flex items-center space-x-2">
      ...
      <div onClick={handleSetToggleSearch} className={`${toggleSearch ? 'block' : '-z-10 absolute translate-x-11'} transition-all text-slate-500 flex items-center justify-center w-10 h-10 hover:bg-slate-100 cursor-pointer rounded-full`}>
        <i className="fa-solid fa-arrow-left"></i>
      </div>
      <div className={`bg-slate-100 shadow-sm h-auto flex items-center rounded-full lg:px-3 px-1.5 py-1`}>
         <i onClick={handleSetToggleSearch} className={`${toggleSearch ? '-ml-5 -z-50 opacity-40' : 'ml-0 z-50'} fas fa-search transition-all text-gray-400 lg:translate-x-0.5 translate-x-2`}></i>
         <input onClick={()=>setToggleSearch(true)} className={`transition-all bg-transparent  outline-none ml-2 text-gray-500 p-1 lg:w-full ${toggleSearch ? 'w-full' : 'w-0' }`} placeholder="Search Facebook" type="text" />
        <div className={`${toggleSearch ? 'block' : 'hidden'} absolute left-0 p-6 bg-white top-14 -translate-x-2 rounded-md`}>
            <UserSearch />
        </div>
      </div>
   </div>
